...
fn lcg(seed: &mut u32) -> u32 { *seed = seed.wrapping_mul(1664525).wrapping_add(1013904223); *seed }
...
fn build_world(n: usize, cs: f32, tighten: bool, seed0: u32) -> (PhysicsWorld, usize) {
    ...
    for i in 0..n {
        ...
        if i % 2 == 0 { world.push_aabb(Vec2::new(rx, ry), Vec2::splat(0.5), Vec2::new(vx, vy), mask, None); }
        else { world.push_circle(Vec2::new(rx, ry), 0.5, Vec2::new(vx, vy), mask, None); }
    }
    (world, seed as usize)
}
...
fn main() {
    ...
    println!("n,cell_size,tighten,end_frame_ms,generate_ms,cells,candidate_pairs,unique_pairs,events");
    for &n in &n_vals {
        for &cs in &sizes {
            for &tighten in &tighten_opts {
                ...
                println!("{},{},{},{:.3},{:.3},{},{},{},{}", n, cs, tighten as u8, t.end_frame_ms, t.generate_ms, stats.cells, stats.candidate_pairs, stats.unique_pairs, events);
            }
        }
    }
}
